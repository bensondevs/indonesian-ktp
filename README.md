...
`parsed()` returns a [`Parsed`](src/NIK/Parsed.php) snapshot (read-only fields from the NIK).

| Method | Role |
| --- | --- |
| `raw()` | Normalized 16-digit string |
| `structureValid()` | Structural segment checks |
| `districtCode()` | Six-digit wilayah key from the NIK |
...
| `age($asOf)`, `isAtLeastYears($min, $asOf)`, `isSeventeenOrOlder($asOf)`, `isTwentyOneOrOlder($asOf)` | Age helpers (conservative when ambiguous) |
...
$parsed->districtCode();              // six-digit wilayah key from NIK
